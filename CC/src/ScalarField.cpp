...
void ScalarField::computeMeanAndVariance(ScalarType &mean, ScalarType* variance, ScalarType &max, ScalarType &min) const
{
	double _mean = 0.0;
	double _std2 = 0.0;
	std::size_t count = 0;
	...
	if (count)
	{
		...
		mean = static_cast<ScalarType>(_mean);
         max= static_cast<ScalarType>(_max);
		min = static_cast<ScalarType>(_min);
		if (variance)
		{
