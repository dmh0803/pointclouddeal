	...
	unsigned step = 1)
{
	...
	if (step == 1)
		cloud->setName("unnamed - 点云");
	else
		cloud->setName(QString("unnamed - 点云 (部分 %1)").arg(step));
	...
	bool showLabelsIn2D/*=false*/)
{
	...
	{
		pDlg.reset(new ccProgressDialog(true, parameters.parentWidget));
		pDlg->setMethodTitle(QObject::tr("打开ASCII文件 [%1]").arg(filename));
		pDlg->setInfo(QObject::tr("估计点数: %1").arg(approximateNumberOfLines));
		pDlg->start();
	}
