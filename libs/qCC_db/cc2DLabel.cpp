...
static const QString CENTER_STRING = QObject::tr("中心");
...
QString cc2DLabel::PickedPoint::prefix(const char* pointTag) const
{
	...
	{
		return QString("点 #") + pointTag;
	}
	else if (_mesh)
	{
		return QString("点@Tri#") + pointTag;
	}
...
QString cc2DLabel::getTitle(int precision) const
{
	...
	else if (count == 2)
	{
		...
		title = QString("距离 %1").arg(dist, 0, 'f', precision);
	}
	else if (count == 3)
	{
		LabelInfo3 info;
		...
		title = QString("面积: %1   拟合外接圆半径R：2%").arg(info.area, 0, 'f', precision).arg(info.r, 0, 'f', precision);
	}
...
void cc2DLabel::updateName()
{
	...
	case 2:
	{
		...
		{
			setName(QString("向量 ") + m_pickedPoints[0].prefix(POINT_INDEX_0)
				+ QString(" - ") + m_pickedPoints[1].prefix(POINT_INDEX_1));
		}
		else
		{
			setName(QString("向量 ") + m_pickedPoints[0].prefix(POINT_INDEX_0) + QString("@") + ENTITY_INDEX_0
	...
	case 3:
	{
		...
		{
			setName(QString("三元 ") + m_pickedPoints[0].prefix(POINT_INDEX_0)
				+ QString(" - ") + m_pickedPoints[1].prefix(POINT_INDEX_1)
				+ QString(" - ") + m_pickedPoints[2].prefix(POINT_INDEX_2));
		}
		else
		{
			setName(QString("三元 ") + m_pickedPoints[0].prefix(POINT_INDEX_0) + QString("@") + ENTITY_INDEX_0
				+ QString(" - ") + m_pickedPoints[1].prefix(POINT_INDEX_1) + QString("@") + ENTITY_INDEX_1
...
void cc2DLabel::getLabelInfo3(LabelInfo3& info) const
{
	...
	//edges length
	info.edges.u[0] = P1P2.normd();  //edge 1-2  AB
	info.edges.u[1] = P2P3.normd();  //edge 2-3  BC
	info.edges.u[2] = P1P3.normd();  //edge 3-1  CA
	...
	//弧长
	info.r = P1P2.normd()*P2P3.normd()*P1P3.normd() / (4 * info.area);//拟合的外接圆半径
	info.arcs.u[0] = 2 * info.r*asin(P1P2.normd() / 2 / info.r);
	info.arcs.u[1] = 2 * info.r*asin(P2P3.normd() / 2 / info.r);
	info.arcs.u[2] = 2 * info.r*asin(P1P3.normd() / 2 / info.r);
...
QStringList cc2DLabel::getLabelContent(int precision) const
{
	...
	case 1: //point
	{
		...
		if (info.hasNormal)
		{
			QString normStr = QString("法线: (%1;%2;%3)").arg(info.normal.x, 0, 'f', precision).arg(info.normal.y, 0, 'f', precision).arg(info.normal.z, 0, 'f', precision);
			body << normStr;
		}
		//color
		if (info.hasRGB)
		{
			QString colorStr = QString("颜色: (%1;%2;%3;%4)").arg(info.color.r).arg(info.color.g).arg(info.color.b).arg(info.color.a);
			body << colorStr;
		}
		//scalar field
		if (info.hasSF)
	...
	case 3: //triangle/plane
	{
		...
		//area
		QString areaStr = QString("面积 = %1").arg(info.area, 0, 'f', precision);
		body << areaStr;
		...
		//angles
		QString angleStr = QString("角度: A=%1 - B=%2 - C=%3 deg.")
		...
		//edges
		QString edgesStr = QString("边长: AB=%1 - BC=%2 - CA=%3")
			.arg(info.edges.u[0], 0, 'f', precision)
			.arg(info.edges.u[1], 0, 'f', precision)
			.arg(info.edges.u[2], 0, 'f', precision);
		body << edgesStr;
		//r外接圆半径
		QString rStr = QString("外接圆半径：R= %1").arg(info.r, 0, 'f', precision);
		body << rStr;
		//arcs
		QString arcsStr = QString("弧长: 弧AB=%1 - 弧BC=%2 - 弧CA=%3")
			.arg(info.arcs.u[0], 0, 'f', precision)
			.arg(info.arcs.u[1], 0, 'f', precision)
			.arg(info.arcs.u[2], 0, 'f', precision);
		body << arcsStr;
	}
	break;
...
void cc2DLabel::drawMeOnly3D(CC_DRAW_CONTEXT& context)
{
	...
	case 1:
	{
		//display point marker as spheres
		{
			...
			{
				c_unitPointMarker = QSharedPointer<ccSphere>(new ccSphere(1.0f, nullptr, "标志点", 12));
...
struct Tab
{
	...
	Tab(int _maxBlockPerRow = 2)
		: maxBlockPerRow(_maxBlockPerRow)//大格子列数
		, blockCount(0)///计数
		, rowCount(0)//小的格子行数
		, colCount(0)
	{}
	...
	**/
	...
	int add2x3Block()
	{
		...
		if (colCount < maxBlockPerRow * 2)///变为2（每个小格子两列）*总的格子列数
		{
			colCount += 2;
			colContent.resize(colCount);
			colWidth.resize(colCount, 0);
		}
		int blockCol = (blockCount % maxBlockPerRow);//maxBlockPerRow=2时，该值为0或1 （一行超过maxBlockPerRow个格子就换行
		//add new row
		if (blockCol == 0)//每次换行即到了第0列时新建三小行
			rowCount += 3;
		++blockCount;

		//return the first column index of the block
		return blockCol * 2;//maxBlockPerRow=2时,该值为0或2
	}
			...
			if (m_showFullBody)
			{
				...
				try
				{
					...
					else if (count == 3)
					{
						LabelInfo3 info;
						getLabelInfo3(info);
						tab.setMaxBlockPerRow(3); //square tab (2x2 blocks)

						//next block: indexes
						{
							int c = tab.add2x3Block();//0
							tab.colContent[c] << "点A"; tab.colContent[c + 1] << (m_pickedPoints[0].itemTitle());
							tab.colContent[c] << "点B"; tab.colContent[c + 1] << (m_pickedPoints[1].itemTitle());
							tab.colContent[c] << "点C"; tab.colContent[c + 1] << (m_pickedPoints[2].itemTitle());
						}
						//next block: edges length
						{
							int c = tab.add2x3Block();//2
						...
						{
							int c = tab.add2x3Block();//0
							tab.colContent[c] << "角A"; tab.colContent[c + 1] << QString::number(info.angles.u[0], 'f', precision);
							tab.colContent[c] << "角B"; tab.colContent[c + 1] << QString::number(info.angles.u[1], 'f', precision);
							tab.colContent[c] << "角C"; tab.colContent[c + 1] << QString::number(info.angles.u[2], 'f', precision);
						}
						//next block: normal
						{
							int c = tab.add2x3Block();//2
							tab.colContent[c] << "弧AB"; tab.colContent[c + 1] << QString::number(info.arcs.u[0], 'f', precision);
							tab.colContent[c] << "弧BC"; tab.colContent[c + 1] << QString::number(info.arcs.u[1], 'f', precision);
							tab.colContent[c] << "弧CA"; tab.colContent[c + 1] << QString::number(info.arcs.u[2], 'f', precision);
						}
						//next block: normal
						{
							int c = tab.add2x3Block();//0
