...
#endif
	//GL window title
	setWindowTitle(QString("三维视图 %1").arg(m_uniqueID));
...
bool ccGLWindow::initialize()
{
	...
	if (!m_initialized)
	{
		...
		if (!m_silentInitialization)
		{
			ccLog::Print("[三维视图 %i] Graphics card manufacturer: %s", m_uniqueID, vendorName);
			ccLog::Print("[三维视图 %i] Renderer: %s", m_uniqueID, glFunc->glGetString(GL_RENDERER));
			ccLog::Print("[三维视图 %i] GL version: %s", m_uniqueID, glFunc->glGetString(GL_VERSION));
			ccLog::Print("[三维视图 %i] GLSL Version: %s", m_uniqueID, glFunc->glGetString(GL_SHADING_LANGUAGE_VERSION));
		}
		...
		if (context()->hasExtension(QByteArrayLiteral("GL_ARB_vertex_buffer_object")))
		{
			...
			{
				if (!m_silentInitialization)
					ccLog::Warning("[三维视图 %i] VBO支持已被禁用，因为它可能在%s卡上不起作用！您可以在显示设置中手动激活它（风险自负！)", m_uniqueID, vendorName);
				params.useVBOs = false;
			}
			else if (!m_silentInitialization)
			{
				ccLog::Print("[三维视图 %i] VBO可用", m_uniqueID);
			}
		}
		else
		{
		...
		if (!m_shadersEnabled)
		{
			...
			if (!m_silentInitialization)
				ccLog::Warning("[三维视图 %i] Shaders and GL filters unavailable", m_uniqueID);
		}
		else
		{
			if (!m_silentInitialization)
				ccLog::Print("[三维视图 %i] 着色器可用", m_uniqueID);

			m_glFiltersEnabled = context()->hasExtension(QByteArrayLiteral("GL_EXT_framebuffer_object"));
			if (m_glFiltersEnabled)
			{
				/*if (!m_silentInitialization)
					ccLog::Print("[3D 视图 %i] GL filters available", m_uniqueID);*/
				m_alwaysUseFBO = true;
			}
			else if (!m_silentInitialization)
			{
				ccLog::Warning("[三维视图 %i] GL filters unavailable (FBO not supported)", m_uniqueID);
			}
			...
			if (!m_colorRampShader)
			{
				...
				{
					if (!m_silentInitialization)
						ccLog::Warning("[三维视图 %i] Not enough memory on shader side to use color ramp shader! (max=%i/%i bytes)", m_uniqueID, maxBytes, minRequiredBytes);
				}
				else
				{
					...
					{
						if (!m_silentInitialization)
							ccLog::Warning(QString("[三维视图 %1] Failed to load color ramp shader: '%2'").arg(m_uniqueID).arg(error));
					...
					{
						if (!m_silentInitialization)
							ccLog::Print("[三维视图 %i] 颜色渐变着色器加载成功", m_uniqueID);
			...
			if (!m_silentInitialization)
			{
				...
				//ccLog::Print(QString("[三维视图 %1] Stereo mode: %2").arg(m_uniqueID).arg(isStereoEnabled ? "supported" : "not supported"));
			}
		}
		...
		{
			if (!m_silentInitialization)
				ccLog::Print("[三维视图 %i] GL KHR (debug) extension available", m_uniqueID);
		...
		if (QGLFormat::openGLVersionFlags() & QGLFormat::OpenGL_Version_3_3)
		{
			...
			if (vaEnabled && !m_customRenderingShader)
			{
				...
				{
					if (!m_silentInitialization)
						ccLog::Warning(QString("[三维视图 %i] Failed to load custom rendering shader: '%2'").arg(m_uniqueID).arg(error));
...
void ccGLWindow::resizeGL(int w, int h)
{
	...
	displayNewMessage(QString("新尺寸= %1 * %2 (px)").arg(m_glViewport.width()).arg(m_glViewport.height()),
				...
				if (ent->isKindOf(CC_TYPES::POINT_CLOUD))
				{
					...
					if (firstCloudWithoutOctree && !cloud->getOctree() && cloud->size() > MIN_POINTS_FOR_OCTREE_COMPUTATION) //no need to use the octree for a few points!
					{
						...
						case ccGui::ParamStruct::ASK_USER:
						{
							...
							QPushButton* yes = new QPushButton("是");
							question.addButton(yes, QMessageBox::AcceptRole);
							QPushButton* no = new QPushButton("否");
							question.addButton(no, QMessageBox::RejectRole);
							QPushButton* always = new QPushButton("总是");
							question.addButton(always, QMessageBox::AcceptRole);
							QPushButton* never = new QPushButton("从不");
...
void ccGLWindow::setPointSize(float size, bool silent/*=false*/)
{
	...
	if (m_viewportParams.defaultPointSize != newSize)
	{
		...
		if (!silent)
		{
			displayNewMessage(QString("新默认点大小: %1").arg(newSize),
				ccGLWindow::LOWER_LEFT_MESSAGE,
				false,
				2,
	...
	else
	{
		if (!silent)
		{
			if (size < MIN_POINT_SIZE_F)
				ccLog::Print(QString("默认点大小已达到最小值 : %1").arg(MIN_POINT_SIZE_F));
			else
				ccLog::Print(QString("默认点大小已达到最大值 : %1").arg(MAX_POINT_SIZE_F));
		}
	}
}
...
void ccGLWindow::setPerspectiveState(bool state, bool objectCenteredView)
{
	...
	if (m_viewportParams.perspectiveView)
	{
		...
		//display message
		displayNewMessage(objectCenteredView ? "中心视角开启" : "泡沫视图开启",
			ccGLWindow::LOWER_LEFT_MESSAGE,
			false,
			2,
