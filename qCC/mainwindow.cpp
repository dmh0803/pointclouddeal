	...
	, m_pfDlg(nullptr)
{
	...
	//setWindowTitle(QString::fromUtf16(u"三维点云分析测量与重构"));
	...
	m_UI->menuFile->insertMenu(m_UI->menu_5->menuAction(), m_recentFiles->menu());
	...
	QMainWindow::statusBar()->showMessage(QString("就绪"));
...
//	addToolBar(Qt::RightToolBarArea, m_pluginUIManager->glFiltersToolbar());////!!!添加右侧工具栏
//	addToolBar(Qt::RightToolBarArea, m_pluginUIManager->mainPluginToolbar());
	...
	// Set up dynamic menus
	m_UI->menubar->insertMenu(m_UI->menu_8->menuAction(), m_pluginUIManager->pluginMenu());///////!!!!!!!!!!终于找到你
	//	m_UI->menuDisplay->insertMenu( m_UI->menuActiveScalarField->menuAction(), m_pluginUIManager->shaderAndFilterMenu() );
	m_pluginUIManager->pluginMenu()->addAction(m_UI->actionComputeGeometricFeature);/////添加贪婪三角法重构
...
void MainWindow::connectActions()
{
	...
	connect(m_UI->actionBoundary, &QAction::triggered, this, &MainWindow::doActionBoundary);
	//"Tools > Clean" menu
	...
	connect(m_UI->actionFitCylindrical, &QAction::triggered, this, &MainWindow::doActionFitCylindrical);
...
{
	bool ok;
	int pointCount = QInputDialog::getInt(this, "用八叉树重采样", "点（估计）", 1000000, 1, INT_MAX, 100000, &ok);
	if (!ok)
		return;
...
void MainWindow::applyTransformation(const ccGLMatrixd& mat)
{
	...
	ccLog::Print("[高级变换] 应用的变换矩阵：");
	ccLog::Print(transMat.toString(12, ' ')); //full precision
	ccLog::Print("提示：复制 (CTRL+C) 并应用它 - 或者它的反向 - 使用“编辑>高级变换”工具在任何实体上");
...
void MainWindow::doActionEditGlobalShiftAndScale()
{
	...
	ccLog::Print("[全局变换/缩放] 新变换: (%f, %f, %f)", shift.x, shift.y, shift.z);
	ccLog::Print("[全局变换/缩放] 新缩放: %f", scale);
		...
		for (auto &entity : shiftedEntities)
		{
			...
			if (preserveGlobalPos)
			{
				...
				if (T.norm() > ZERO_TOLERANCE || std::abs(scaleCoef - 1.0) > ZERO_TOLERANCE)
				{
					...
					ccLog::Warning(QString("[全局变换/缩放] 为了保留其原始位置，实体 '%1' 已被转换为 (%2,%3,%4) 并重新缩放了因子 %5")
						.arg(ent->getName())
						.arg(T.x)
						.arg(T.y)
						.arg(T.z)
...
void MainWindow::doActionComputePointsVisibility()
{
	...
	if (sf)
	{
		...
		pdlg.setMethodTitle(tr("计算可见性"));
		pdlg.setInfo(tr("点数: %L1").arg(pointCloud->size()));
		pdlg.start();
...
void MainWindow::doRemoveDuplicatePoints()
{
	...
	bool ok;
	minDistanceBetweenPoints = QInputDialog::getDouble(this, "清楚重复点", "最小点间距:", minDistanceBetweenPoints, 0, 1.0e8, 12, &ok);
	if (!ok)
		return;
	...
	for (ccHObject *entity : selectedEntities)
	{
		...
		if (cloud)
		{
			...
			if (result == CCLib::GeometricalAnalysisTools::NoError)
			{
				...
				if (duplicateCount == 0)
				{
					ccConsole::Print(QString("Cloud '%1' 没有重复点").arg(cloud->getName()));
				}
				else
				{
					ccConsole::Warning(QString("点云 '%1' 有 %2 个重复点").arg(cloud->getName()).arg(duplicateCount));
	...
	ccHObject::Container results;
	{
		for (auto &item : toFilter)
		{
			...
			if (resultOutside)
			{
				...
				resultOutside->setName(resultOutside->getName() + ".外面的");
	...
	if (!clouds.empty())
	{
		...
		for (size_t i = 0; i < clouds.size(); ++i)
		{
			...
			if (!firstCloud)
			{
				...
				if (QMessageBox::question(this, "原始点云索引", "是否要使用原始点云索引生成标量字段?") == QMessageBox::Yes)
				{
...
void MainWindow::doActionRegister()
{
		...
		this))
	{
		...
		if (pc)
		{
			...
			data->setName(data->getName() + QString(".已注册"));
			zoomOn(data);
		}
...
void MainWindow::doAction4pcsRegister()
{
		...
		nbMaxCandidates))
	{
		...
		newDataCloud->setName(data->getName() + QString(".已注册"));
...
void MainWindow::doActionSubsample()
{
	...
	ccHObject::Container resultingClouds;
	{
		...
		for (size_t i = 0; i < clouds.size(); ++i)
		{
			...
			if (!sampledCloud)
			{
				ccConsole::Warning(QString("[采样] 采样点云'%1'失败!").arg(cloud->getName()));
				errors = true;
				continue;
			}
			...
			if (newPointCloud)
			{
				newPointCloud->setName(cloud->getName() + QString(".采样后"));
				...
				if (warnings)
				{
					ccLog::Warning("[采样]内存不足：颜色、法线或标量字段可能丢失!");
					errors = true;
				}
			}
			else
		...
		ccLog::Print("[采样] 计时: %3.3f s.", eTimer.elapsed() / 1000.0);

		if (errors)
		{
			ccLog::Error("发生错误(请查看控制台)");
		}
	}
...
void MainWindow::doActionBoundary()
{
	PointT point;
	...
	for (ccHObject *entity : selectedEntities)
	{
		...
		ccPointCloud* cloud0 = static_cast<ccPointCloud*>(entity);//CC类型的点云
		int num = cloud0->size();
		for (int i = 0; i < num; i++)

		{
			point.x = (cloud0->getPoint(i))->x;

			point.y = (cloud0->getPoint(i))->y;

			point.z = (cloud0->getPoint(i))->z;
			cloud->push_back(point);//CC类型的点云转为PCL点云
		}
		...
		est.setKSearch(50);  //数值越高，最终边界识别的精度越好
		//  est.setRadiusSearch(everagedistance);  //搜索半径
		est.compute(boundaries);

		ccPointCloud* boundPoints = new ccPointCloud();//是边界点
		PointCloudT noBoundPoints;//不是
		int countBoundaries = 0;
		for (int i = 0; i < cloud->size(); i++)
		{
			CCVector3 P;
			uint8_t x = (boundaries.points[i].boundary_point);
			int a = static_cast<int>(x); //强制类型转换
			if (a == 1)
			{
				P.x = cloud->points[i].x;
				P.y = cloud->points[i].y;
				P.z = cloud->points[i].z;
				boundPoints->addPoint(P);
				countBoundaries++;
			}
			else
		...
		if (boundPoints)
		{
			boundPoints->setName(cloud0->getName() + QString(".边界"));
			boundPoints->setDisplay(cloud0->getDisplay());
			if (cloud0->getParent())
				cloud0->getParent()->addChild(boundPoints);
			addToDB(boundPoints);
		}
	}
	refreshAll();
...
void MainWindow::doActionLabelConnectedComponents()
{
	...
	if (!dlg.exec())
		return;
	float translate = dlg.getOctreeLevel();
	...
	for (ccHObject *entity : getSelectedEntities())
	{
		...
		{
			pointT.x = (cloud->getPoint(i))->x;

			pointT.y = (cloud->getPoint(i))->y;

			pointT.z = (cloud->getPoint(i))->z;
			ground->push_back(pointT);
		}
		Eigen::Matrix4f transform_1 = Eigen::Matrix4f::Identity();
		transform_1(2, 3) = translate;
		pcl::PointCloud<pcl::PointXYZ>::Ptr transformed1(new pcl::PointCloud<pcl::PointXYZ>);
		pcl::transformPointCloud(*ground, *transformed1, transform_1);
		int numb = transformed1->points.size();
		cloud->clear();
		cloud->reserve(static_cast<unsigned>(numb));
		for (int i = 0; i < numb; i++)

		{
			CCVector3 P11(transformed1->points[i].x, transformed1->points[i].y, transformed1->points[i].z);
			cloud->addPoint(P11);
		}
		entity = cloud;
	}
...
void MainWindow::doActionFitCylindrical() ///////拟合圆柱
{
	...
	for (ccHObject *entity : getSelectedEntities())
	{
		...
		normalEstimator.setInputCloud(ground);
		...
		seg.setRadiusLimits(0, 50);				// radius is within 8 centimeters
		seg.setInputCloud(ground);
		seg.setInputNormals(normalsFilter);

		// Perform segment
		seg.segment(*inliers_cylinder, *coeffients_cylinder);
		Eigen::Vector3f angle_before;
		angle_before[0] = coeffients_cylinder->values[3];
		angle_before[1] = coeffients_cylinder->values[4];
		angle_before[2] = coeffients_cylinder->values[5];
		angle_before.normalize();
		Eigen::Vector3f angle_after;
		angle_after[0] = 0;
		angle_after[1] = 0;
		angle_after[2] = 1;
		...
		pcl::transformPointCloud(*ground, *transformed, rotationMatrix);
		int numa = transformed->points.size();
		cloud->clear();
		cloud->reserve(static_cast<unsigned>(numa));

		for (int i = 0; i < numa; i++)

		{
			CCVector3 P11(transformed->points[i].x, transformed->points[i].y, transformed->points[i].z);
			cloud->addPoint(P11);
		}
		entity = cloud;
		Eigen::Vector4f centroid;
		pcl::compute3DCentroid(*ground, centroid);
		radius = coeffients_cylinder->values[6];
		center.x = centroid[0];
		center.y = centroid[1];
		center.z = centroid[2];
		CCVector3 normal;
		normal.x = coeffients_cylinder->values[3];
		normal.y = coeffients_cylinder->values[4];
		normal.z = coeffients_cylinder->values[5];
		double rms;
		...
		*/
		ccLog::Print(QString("[拟合圆柱] 点云 '%1': 轴向 (%2,%3,%4) - 半径 = %5 [RMS = %6]")
			.arg(cloud->getName())
			.arg(normal.x)
			.arg(normal.y)
			.arg(normal.z)
		...
		ccCylinder *cylinder = new ccCylinder(radius, 5, &trans, QString("圆柱 r=%1 [rms %2]").arg(radius));
		cloud->addChild(cylinder);
		//sphere->setDisplay(cloud->getDisplay());
		cylinder->prepareDisplayForRefresh();
		addToDB(cylinder, false, false, false);
	}
...
void MainWindow::doActionMatchBBCenters()
{
	...
	////TODO: maybe the user would like to select the reference himself ;)
	//ccHObject* refEnt = selectedEntities[0];
	//ccPointCloud* cloud0 = ccHObjectCaster::ToPointCloud(selectedEntities[0]);
	//ccPointCloud* cloud1 = ccHObjectCaster::ToPointCloud(selectedEntities[1]);
	////求第一个圆柱的法向量
	//pcl::PointXYZ pointTa;
	//pcl::PointCloud<pcl::PointXYZ>::Ptr ground0(new pcl::PointCloud<pcl::PointXYZ>());//待拟合点云指针
	//int num0 = cloud0->size();
	//for (int i = 0; i < num0; i++)
	//{
	//	pointTa.x = (cloud0->getPoint(i))->x;

	//	pointTa.y = (cloud0->getPoint(i))->y;

	//	pointTa.z = (cloud0->getPoint(i))->z;
	//	ground0->push_back(pointTa);
	//}
	//// Create segmentation object for cylinder segmentation and set all the parameters
	//pcl::ModelCoefficients::Ptr coeffients_cylinder0(new pcl::ModelCoefficients);
	//pcl::PointIndices::Ptr inliers_cylinder0(new pcl::PointIndices);

	////  Normals
	//pcl::search::Search<pcl::PointXYZ>::Ptr tree0 = boost::shared_ptr<pcl::search::Search<pcl::PointXYZ>>(new pcl::search::KdTree<pcl::PointXYZ>);
	//pcl::PointCloud<pcl::Normal>::Ptr normalsFilter0(new pcl::PointCloud<pcl::Normal>);
	//pcl::NormalEstimation<pcl::PointXYZ, pcl::Normal> normalEstimator0;
	//normalEstimator0.setSearchMethod(tree0);
	//normalEstimator0.setInputCloud(ground0);
	//normalEstimator0.setKSearch(30);
	//normalEstimator0.compute(*normalsFilter0);

	//pcl::SACSegmentationFromNormals<pcl::PointXYZ, pcl::Normal> seg0;
	//seg0.setOptimizeCoefficients(true);
	//seg0.setModelType(pcl::SACMODEL_CYLINDER);
	//seg0.setMethodType(pcl::SAC_RANSAC);
	////seg.setNormalDistanceWeight(0.1);
	//seg0.setNormalDistanceWeight(0.2);		// for coarse data
	//seg0.setMaxIterations(1000);
	//seg0.setDistanceThreshold(0.3);
	//seg0.setRadiusLimits(0, 6);				// radius is within 8 centimeters
	//seg0.setInputCloud(ground0);
	//seg0.setInputNormals(normalsFilter0);

	//// Perform segment
	//seg0.segment(*inliers_cylinder0, *coeffients_cylinder0);

	////radius = coeffients_cylinder->values[6];
	////
	//Eigen::Vector3f angle_before;
	//angle_before[0] = coeffients_cylinder0->values[3];
	//angle_before[1] = coeffients_cylinder0->values[4];
	//angle_before[2] = coeffients_cylinder0->values[5];
	//angle_before.normalize();
	//Eigen::Vector3f angle_after;
	//angle_after[0] = 0;
	//angle_after[1] = 0;
	...
	//rotationMatrix(2, 2) = cos(angle) + p_rotate[2] * p_rotate[2] * (1 - cos(angle));
	//pcl::PointCloud<pcl::PointXYZ>::Ptr transformed(new pcl::PointCloud<pcl::PointXYZ>);
	//pcl::transformPointCloud(*ground0, *transformed, rotationMatrix);
	//int numa = transformed->points.size();
	//cloud0->clear();
	//cloud0->reserve(static_cast<unsigned>(numa));
	...
	//selectedEntities[0] = cloud0;

	////求第二个圆柱的法向量

	//pcl::PointXYZ pointT1;
	//pcl::PointCloud<pcl::PointXYZ>::Ptr ground1(new pcl::PointCloud<pcl::PointXYZ>());//待拟合点云指针
	//int num1 = cloud1->size();
	//for (int i = 0; i < num1; i++)
	//{
	//	pointT1.x = (cloud1->getPoint(i))->x;

	//	pointT1.y = (cloud1->getPoint(i))->y;

	//	pointT1.z = (cloud1->getPoint(i))->z;
	//	ground1->push_back(pointT1);
	//}
	//// Create segmentation object for cylinder segmentation and set all the parameters
	//pcl::ModelCoefficients::Ptr coeffients_cylinder1(new pcl::ModelCoefficients);
	...
	//pcl::PointCloud<pcl::Normal>::Ptr normalsFilter1(new pcl::PointCloud<pcl::Normal>);
	//pcl::NormalEstimation<pcl::PointXYZ, pcl::Normal> normalEstimator1;
	//normalEstimator1.setSearchMethod(tree1);
	//normalEstimator1.setInputCloud(ground1);
	//normalEstimator1.setKSearch(30);
	//normalEstimator1.compute(*normalsFilter1);
	...
	////seg.setNormalDistanceWeight(0.1);
	//seg1.setNormalDistanceWeight(0.2);		// for coarse data
	//seg1.setMaxIterations(1000);
	//seg1.setDistanceThreshold(0.3);
	//seg1.setRadiusLimits(0, 6);				// radius is within 8 centimeters
	//seg1.setInputCloud(ground1);
	//seg1.setInputNormals(normalsFilter1);

	//// Perform segment
	//seg1.segment(*inliers_cylinder1, *coeffients_cylinder1);
	////旋转第二个圆柱向量到x轴
	//Eigen::Vector3f angle_before1;
	//angle_before1[0] = coeffients_cylinder1->values[3];
	//angle_before1[1] = coeffients_cylinder1->values[4];
	//angle_before1[2] = coeffients_cylinder1->values[5];
	//angle_before1.normalize();
	//float angle1 = acos(angle_before.dot(angle_after));//点积，得到两向量的夹角
	//Eigen::Vector3f p_rotate1 = angle_before.cross(angle_after);//叉积，得到的还是向量
	//p_rotate1.normalize();//该向量的单位向量，即旋转轴的单位向量
	//Eigen::Matrix4f rotationMatrix1 = Eigen::Matrix4f::Identity();
	//rotationMatrix1(0, 0) = cos(angle1) + p_rotate1[0] * p_rotate1[0] * (1 - cos(angle1));
	//rotationMatrix1(0, 1) = p_rotate1[0] * p_rotate1[1] * (1 - cos(angle1)) - p_rotate1[2] * sin(angle1);//这里跟公式比多了一个括号，但是看实验结果它是对的。
	//rotationMatrix1(0, 2) = p_rotate1[1] * sin(angle1) + p_rotate1[0] * p_rotate1[2] * (1 - cos(angle1));

	//rotationMatrix1(1, 0) = p_rotate1[2] * sin(angle1) + p_rotate1[0] * p_rotate1[1] * (1 - cos(angle1));
	//rotationMatrix1(1, 1) = cos(angle1) + p_rotate1[1] * p_rotate1[1] * (1 - cos(angle1));
	//rotationMatrix1(1, 2) = -p_rotate1[0] * sin(angle1) + p_rotate1[1] * p_rotate1[2] * (1 - cos(angle1));

	//rotationMatrix1(2, 0) = -p_rotate1[1] * sin(angle1) + p_rotate1[0] * p_rotate1[2] * (1 - cos(angle1));
	//rotationMatrix1(2, 1) = p_rotate1[0] * sin(angle1) + p_rotate1[1] * p_rotate1[2] * (1 - cos(angle1));
	//rotationMatrix1(2, 2) = cos(angle1) + p_rotate1[2] * p_rotate1[2] * (1 - cos(angle1));
	//pcl::PointCloud<pcl::PointXYZ>::Ptr transformed1(new pcl::PointCloud<pcl::PointXYZ>);
	//pcl::transformPointCloud(*ground1, *transformed1, rotationMatrix1);
	//int numb = transformed1->points.size();
	//cloud1->clear();
	//cloud1->reserve(static_cast<unsigned>(numb));
	...
	for (ccHObject *entity : getSelectedEntities())
	{
		...
		pcl::NormalEstimation<pcl::PointXYZ, pcl::Normal> normalEstimator0;
		normalEstimator0.setSearchMethod(tree0);
		normalEstimator0.setInputCloud(ground);
		normalEstimator0.setKSearch(30);
		normalEstimator0.compute(*normalsFilter0);
		...
		seg0.setMaxIterations(1000);
		seg0.setDistanceThreshold(0.3);
		seg0.setRadiusLimits(0, 50);				// radius is within 8 centimeters
		seg0.setInputCloud(ground);
		seg0.setInputNormals(normalsFilter0);
		...
		pcl::transformPointCloud(*transformed, *trans, translation);
		cloud->clear();
		cloud->reserve(static_cast<unsigned>(numa));
		...
		{
			CCVector3 P12(trans->points[i].x, trans->points[i].y, trans->points[i].z);
			cloud->addPoint(P12);
		}
		entity = cloud;
	}
...
void MainWindow::doActionSORFilter()
{
	...
	for (ccHObject *entity : selectedEntities)
	{
		...
		if (selection && cloud)
		{
			...
			{
				ccLog::Warning(QString("[统计滤波] 没有点从点云'%1'删除").arg(cloud->getName()));
			}
			else
			{
				...
				if (cleanCloud)
				{
					cleanCloud->setName(cloud->getName() + QString(".去噪后"));
					...
					if (firstCloud)
					{
						ccConsole::Warning("先前选定的实体（源）已隐藏!");
						firstCloud = false;
				...
				{
					ccConsole::Warning(QString("[统计滤波] 内存不足，无法创建去噪后的点云 '%1'!").arg(cloud->getName()));
				}
			}
		...
		else
		{
			...
			if (cloud != nullptr)
			{
				ccConsole::Warning(QString("[统计滤波] 无法应用去噪器于点云 '%1'! (内存不足?)").arg(cloud->getName()));
			}
			else
			{
				ccConsole::Warning("[统计滤波] 尝试对空点云应用噪声过滤器");
			}
		}
	}
...
void MainWindow::doActionFilterNoise()
{
	...
	for (ccHObject *entity : selectedEntities)
	{
		...
		if (selection && cloud)
		{
			if (selection->size() == cloud->size())
			{
				ccLog::Warning(QString("[去噪] 未从点云'%1'中删除任何点 ").arg(cloud->getName()));
			}
			else
			{
				ccPointCloud* cleanCloud = cloud->partialClone(selection);
				if (cleanCloud)
				{
					cleanCloud->setName(cloud->getName() + QString(".清除后"));
					cleanCloud->setDisplay(cloud->getDisplay());
					if (cloud->getParent())
						cloud->getParent()->addChild(cleanCloud);
					addToDB(cleanCloud);
				...
				{
					ccConsole::Warning(QString("[去噪] 内存不足，无法创建去噪后的点云 '%1'!").arg(cloud->getName()));
				}
			}
		...
		else
		{
			...
			if (cloud != nullptr)
			{
				ccConsole::Warning(QString("[去噪] 无法应用去噪器于点云 '%1'! (内存不足?)").arg(cloud->getName()));
			}
			else
			{
				ccConsole::Warning("[去噪] 尝试对空点云应用噪声过滤器");
			}
		}
	}
...
void MainWindow::doActionUnroll()
{
	...
	if (!cloud || !cloud->isA(CC_TYPES::POINT_CLOUD))
	{
		ccConsole::Error("方法不能应用于锁定的顶点或虚拟点云!");
		return;
	}
	ccPointCloud* pc = static_cast<ccPointCloud*>(cloud);
	...
	{
		QMessageBox::critical(this, "错误", "角度范围无效");
		return;
	}
	...
	if (output)
	{
		if (m_selectedEntities[0]->isA(CC_TYPES::MESH))
		{
			...
			mesh->setEnabled(false);
			ccConsole::Warning("[展开] 原始模型已自动隐藏");
		...
		{
			pc->setEnabled(false);
			ccConsole::Warning("[展开] 原始点云已自动隐藏");
			if (pc->getParent())
			{
...
ccGLWindow* MainWindow::new3DView(bool allowEntitySelection)
{
	...
	QMainWindow::statusBar()->showMessage(QString("新三维视窗"), 2000);
...
void MainWindow::activateRegisterPointPairTool()
{
	...
	if (!win)
	{
		ccLog::Error("[PointPairRegistration] 创建三维视图失败!");
		return;
	}
			...
			if (entity->isKindOf(CC_TYPES::POINT_CLOUD) || entity->isKindOf(CC_TYPES::MESH))
			{
				...
				if (segmentationResult)
				{
					...
					if (!deleteHiddenParts)
					{
						...
						if (!deleteOriginalEntity)
						{
							entity->setName(entity->getName() + QString(".保留的"));
							putObjectBackIntoDBTree(entity, objContext);
						}
					}
					else
...
void MainWindow::zoomOnSelectedEntities()
{
	...
	for (size_t i = 0; i < selNum; ++i)
	{
		...
		if (win)
		{
			...
			{
				ccLog::Error("所有选定的实体必须显示在同一个三维视图中!");
				return;
			}
		}
	}
...
void MainWindow::onItemPicked(const PickedItem& pi)
{
	...
	case PICKING_ROTATION_CENTER:
	{
		...
		if (m_transTool && m_transTool->started())
		{
			...
			s_pickingWindow->displayNewMessage(QString(), ccGLWindow::LOWER_LEFT_MESSAGE, false); //clear previous message
			s_pickingWindow->displayNewMessage(QString("点 (%1 ; %2 ; %3) 设置为交互变换的旋转中心")
...
void MainWindow::doLevel()
{
	...
	if (!win)
	{
		ccConsole::Error("没有活动的三维视图!");
		return;
	}

	if (!haveOneSelection())
	{
		ccConsole::Error("选择一个实体!");
		return;
	}
...
void MainWindow::doActionCrop()
{
	...
	bool successes = false;
	{
		for (ccHObject *entity : candidates)
		{
			...
			if (croppedEnt)
			{
				croppedEnt->setName(entity->getName() + QString(".立方截取后"));
...
void MainWindow::doActionClone()
{
	...
	for (ccHObject *entity : getSelectedEntities())
	{
		...
		if (entity->isKindOf(CC_TYPES::POINT_CLOUD))
		{
			...
			if (!clone)
			{
				ccConsole::Error(QString("复制点云%1时发生错误").arg(entity->getName()));
			}
		}
		else if (entity->isKindOf(CC_TYPES::PRIMITIVE))
		{
			...
			if (!clone)
			{
				ccConsole::Error(QString("复制原始图形%1时发生错误").arg(entity->getName()));
			}
		}
		else if (entity->isA(CC_TYPES::MESH))
		{
			...
			if (!clone)
			{
				ccConsole::Error(QString("复制模型%1时发生错误").arg(entity->getName()));
			}
		}
		else if (entity->isA(CC_TYPES::POLY_LINE))
		{
			...
			if (!clone)
			{
				ccConsole::Error(QString("复制折现%1时发生错误").arg(entity->getName()));
			}
		}
		...
		{
			ccLog::Warning(QString("实体 '%1' 不能被复制(该类型不支持!)").arg(entity->getName()));
		}
...
void MainWindow::doActionFitSphere()
{
	...
	for (ccHObject *entity : getSelectedEntities())
	{
		...
		if (CCLib::GeometricalAnalysisTools::DetectSphereRobust(cloud,
			...
			confidence) != CCLib::GeometricalAnalysisTools::NoError)
		{
			ccLog::Warning(QString("[球拟合] 点云 '%1'拟合球失败").arg(cloud->getName()));
			continue;
		}

		ccLog::Print(QString("[球拟合] 点云 '%1': 中心 (%2,%3,%4) - 半径 = %5 [RMS = %6]")
			.arg(cloud->getName())
			.arg(center.x)
			.arg(center.y)
			.arg(center.z)
		...
		ccGLMatrix trans;
		trans.setTranslation(center);
		ccSphere* sphere = new ccSphere(radius, &trans, QString("球 r=%1 [rms %2]").arg(radius).arg(rms));
...
void MainWindow::doComputePlaneOrientation(bool fitFacet)
{
	...
	for (ccHObject *entity : selectedEntities)
	{
		...
		if (cloud)
		{
			...
			if (plane)
			{
				ccConsole::Print(QString("[方向] 实体 '%1'").arg(entity->getName()));
				ccConsole::Print("\t- 平面拟合RMS: %f", rms);

				//We always consider the normal with a positive 'Z' by default!
				if (N.z < 0.0)
					N *= -1.0;
				ccConsole::Print("\t- 法线: (%f,%f,%f)", N.x, N.y, N.z);
				float x = N.x;
				float y = N.y;
				float z = N.z;
				ofstream ofs;
				...
				ccConsole::Print("[方向]一个能使平面水平的矩阵（法线指向 Z+) 是:");
				ccConsole::Print(makeZPosMatrix.toString(12, ' ')); //full precision
				ccConsole::Print("[方向] 您可以复制这个矩阵值（CTRL+C）并将它们粘贴到“高级变换”对话框中");
...
void MainWindow::doComputeGeometricFeature()
{
	...
	for (ccHObject *entity : getSelectedEntities())
	{
		...
		ccGeomFeaturesDlg dlg(this);
		if (!dlg.exec())
			return;

		int qnumber = dlg.num();
		float radius1 = dlg.radius();

		//贪婪三角形重建曲面
		pcl::GreedyProjectionTriangulation<pcl::PointNormal> gp3;
		pcl::PolygonMesh mesh;
		/*作用是控制搜索邻域大小。前者定义了可搜索的邻域个数，后者规定了被样本点搜索其邻近点的最远距离，（是为了适应点云密度的变化），特征值一般是50-100和2.5-3（或者1.5每栅格）。*/
		gp3.setSearchRadius(radius1);//连接点之间的最大距离（最大边长）用于确定k近邻的球半径【默认值 0】
		gp3.setMu(2.5f);//近邻距离乘子，以得到每个点的最终搜索半径 0
		gp3.setMaximumNearestNeighbors(qnumber);
		...
		pcl::io::saveVTKFile("mymeshceshi.vtk", mesh);
	...
	const char* name;
	name = "mymeshceshi.vtk";
	QString str = QString::fromUtf8(name);
	QStringList selectedFiles = str.split(";"); // 注意，如果str是空字符串，list1会增加一个空字符串到列表里，其size=1，我为此吃过苦头～
	for (int i = 0; i < selectedFiles.size(); i++)
	{
		QStringList list2 = selectedFiles[i].split(tr(","));
	}
	//file choosing dialog
	//QStringList selectedFiles = "mymeshceshi.vtk";
...
void MainWindow::doActionComputeBestICPRmsMatrix()
{
	...
	{
		ccProgressDialog pDlg(true, this);
		pDlg.setMethodTitle(tr("测试所有可能的位置"));
		pDlg.setInfo(tr("%1 点云 %2 位置").arg(cloudCount).arg(matrices.size()));
	...
	if (execute)
	{
		...
		for (auto &cloud : clouds)
		{
			...
			if (sf)
			{
				...
				sf->computeMinAndMax();
				sf->computeMeanAndVariance(mean, &variance,max,min);
...
void MainWindow::toggleLockRotationAxis()
{
	...
	if (win)
	{
		...
		if (isLocked)
		{
			ccAskThreeDoubleValuesDlg axisDlg("x", "y", "z", -1.0e12, 1.0e12, s_lastAxis.x, s_lastAxis.y, s_lastAxis.z, 4, "锁定旋转轴", this);
		...
		if (isLocked)
		{
			win->displayNewMessage(QString("[旋转轴已锁定]"), ccGLWindow::UPPER_CENTER_MESSAGE, false, 24 * 3600, ccGLWindow::ROTAION_LOCK_MESSAGE);
		}
		else
		{
...
void MainWindow::enableUIItems(dbTreeSelectionInfo& selInfo)
{
	...
	m_UI->actionFitCylindrical->setEnabled(atLeastOneCloud);
	...
	m_UI->actionBoundary->setEnabled(atLeastOneCloud);
	...
	m_UI->actionMatchBBCenters->setEnabled(atLeastOneEntity);
	m_UI->actionMatchScales->setEnabled(atLeastOneEntity);
...
void MainWindow::doActionGlobalShiftSeetings()
{
	...
	ccLog::Print(QString("[全局变换] 最大坐标绝对值 = %1 / 最大对角线绝对值 = %2")
