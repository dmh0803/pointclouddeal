...
bool CommandOctreeNormal::process(ccCommandLineInterface &cmd)
{
	...
	for (const CLCloudDesc& thisCloudDesc : cmd.clouds())
	{
		...
		cloud->setName(cloud->getName() + QObject::tr(".八叉树法线"));
		if (cmd.autoSaveMode())
		{
...
CommandSubsample::CommandSubsample()
	: ccCommandLineInterface::Command(QObject::tr("采样"), COMMAND_SUBSAMPLE)
{}

bool CommandSubsample::process(ccCommandLineInterface &cmd)
{
	cmd.print(QObject::tr("[采样]"));
	if (cmd.clouds().empty())
	{
		return cmd.error(QObject::tr("没有要重新采样的点云（请确保使用 \"-%1 [点云文件名]\" 先于 \"-%2\")").arg(COMMAND_OPEN, COMMAND_SUBSAMPLE));
	}
	...
	if (method == "RANDOM")
	{
		if (cmd.arguments().empty())
		{
			return cmd.error(QObject::tr("缺少参数：后面的点数 \"-%1 RANDOM\"").arg(COMMAND_SUBSAMPLE));
		}
		...
		if (!ok)
		{
			return cmd.error(QObject::tr("随机重采样点数无效！"));
		}
		...
		for (size_t i = 0; i < cmd.clouds().size(); ++i)
		{
			...
			if (!refCloud)
			{
				return cmd.error(QObject::tr("采样过程失败!"));
			}
			cmd.print(QObject::tr("\tResult: %1 个点").arg(refCloud->size()));
			...
			if (result)
			{
				result->setName(cmd.clouds()[i].pc->getName() + QObject::tr(".采样后"));
				if (cmd.autoSaveMode())
				{
...
bool CommandApplyTransformation::process(ccCommandLineInterface &cmd)
{
	...
	if (!cmd.clouds().empty())
	{
		for (const CLCloudDesc& desc : cmd.clouds())
		{
			...
			desc.pc->setName(desc.pc->getName() + ".变换后");
		}
		//save output
	...
	if (!cmd.meshes().empty())
	{
		for (const CLMeshDesc& desc : cmd.meshes())
		{
			...
			desc.mesh->setName(desc.mesh->getName() + ".变换后");
		}
		//save output
...
bool CommandSORFilter::process(ccCommandLineInterface &cmd)
{
	...
	for (size_t i = 0; i < cmd.clouds().size(); ++i)
	{
		...
		if (selection)
		{
			...
			if (cleanCloud)
			{
				cleanCloud->setName(cloud->getName() + QObject::tr(".清除后"));
				if (cmd.autoSaveMode())
				{
...
bool CommandExtractVertices::process(ccCommandLineInterface &cmd)
{
	...
	for (size_t i = 0; i < cmd.meshes().size(); ++i)
	{
		...
		cmd.clouds().emplace_back(pc, cmd.meshes()[i].basename + QObject::tr(".顶点"), cmd.meshes()[i].path);
		...
		if (ref)
		{
			if (ref->size() != 0)
			{
				...
				if (croppedCloud)
				{
					...
					croppedCloud->setName(cmd.clouds()[i].pc->getName() + QObject::tr(".立方截取后"));
					cmd.clouds()[i].basename += "_CROPPED";
					if (cmd.autoSaveMode())
					{
						QString errorStr = cmd.exportEntity(cmd.clouds()[i]);
...
bool CommandDist::process(ccCommandLineInterface &cmd)
{
	...
	if (cmd.clouds().empty())
	{
		...
		if (!m_cloud2meshDist || cmd.meshes().size() < 2)
		{
			...
			return cmd.error(QObject::tr("没有可用的点云。一定要先打开或生成一个!"));
		}
		else
		{
			cmd.warning(QObject::tr("没有可用的点云。将使用第一个网格顶点作为比较云。"));
