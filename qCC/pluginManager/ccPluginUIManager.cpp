...
void ccPluginUIManager::init()
{
	...
	QVector<ccStdPluginInterface *>	coreStdPlugins;//核心
	QVector<ccStdPluginInterface *>	thirdPartyStdPlugins;//第三方
	...
	for (ccPluginInterface *plugin : plugins)
	{
		...
		{
		case CC_STD_PLUGIN: //standard plugin  PCL和泊松都是标准插件
		{
		...
		case CC_GL_FILTER_PLUGIN: //GL filter（无用）
		{
	...
	for (ccStdPluginInterface *plugin : coreStdPlugins)//将核心插件添加至菜单栏和工具栏
	{
		QList<QAction *> actions = plugin->getActions();

		addActionsToMenu(plugin, actions);
		//addActionsToToolBar(plugin, actions);暂时不放到工具栏
...
void ccPluginUIManager::setupActions()
{
	...
	m_showPluginToolbar = new QAction(tr("重构"), this);
...
{
	m_pluginMenu = new QMenu(tr("重构"), m_parentWidget);
...
{
	// If the plugin has more than one action we create its own menu////如果该功能有两个以上选项
	if (actions.size() > 1)
	{
		QMenu	*menu = new QMenu(stdPlugin->getName(), m_parentWidget);///第二分支
		...
		m_pluginMenu->addMenu(menu);///给plugins添加子分支
	}
	else // otherwise we just add it to the main menu如果只有一个选项，如泊松重构
	{
...
{
	m_mainPluginToolbar = new QToolBar(tr("重构"), m_parentWidget);
...
void ccPluginUIManager::enableGLFilter()
{
	...
	if (win == nullptr)
	{
		ccLog::Warning("[GL filter] 没有活动的三维视图");
		return;
	}
