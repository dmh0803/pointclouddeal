...
ccDBRoot::ccDBRoot(ccCustomQTreeView* dbTreeWidget, QTreeView* propertiesTreeWidget, QObject* parent) : QAbstractItemModel(parent)
{
	...
	m_expandBranch = new QAction("展开分支", this);
	m_collapseBranch = new QAction("折叠分支", this);
	m_gatherInformation = new QAction("信息（递归）", this);
	m_sortChildrenType = new QAction("按类型对子项排序", this);
	m_sortChildrenAZ = new QAction("按名称对子项进行排序(A-Z)", this);
	m_sortChildrenZA = new QAction("按名称对子项进行排序(Z-A)", this);
	m_selectByTypeAndName = new QAction("按类型和/或名称选择子项", this);
	m_deleteSelectedEntities = new QAction("删除", this);
	m_toggleSelectedEntities = new QAction("切换", this);
	m_toggleSelectedEntitiesVisibility = new QAction("切换可见性", this);
	m_toggleSelectedEntitiesColor = new QAction("切换颜色", this);
	m_toggleSelectedEntitiesNormals = new QAction("切换法线", this);
	m_toggleSelectedEntitiesMat = new QAction("切换材质/纹理", this);
	//m_toggleSelectedEntitiesSF = new QAction("Toggle SF", this);
	m_toggleSelectedEntities3DName = new QAction("切换3D名称", this);
	m_addEmptyGroup = new QAction("添加空组", this);
	...
	m_enableBubbleViewMode = new QAction("泡沫视图", this);
...
void ccDBRoot::deleteSelectedEntities()
{
	...
	for (unsigned i = 0; i < selCount; ++i)
	{
		...
		{
			ccLog::Warning(QString("目标 '%1' 无法以这种方式删除（锁定）").arg(obj->getName()));
			continue;
		}
...
void ccDBRoot::alignCameraWithEntity(bool reverse)
{
	...
	//output the transformation matrix that would make this normal points towards +Z
	{
		...
		ccLog::Print("[方向] 您可以复制这个矩阵值（CTRL+C）并将它们粘贴到“应用转换工具”对话框中");
	}
}
...
void ccDBRoot::selectByTypeAndName()
{
	...
	scDlg.addType("点云", CC_TYPES::POINT_CLOUD);
	scDlg.addType("折线", CC_TYPES::POLY_LINE);
	scDlg.addType("模型", CC_TYPES::MESH);
	scDlg.addType("  Sub-mesh", CC_TYPES::SUB_MESH);
	scDlg.addType("原始图形", CC_TYPES::PRIMITIVE);
	scDlg.addType("    平面", CC_TYPES::PLANE);
	scDlg.addType("    球", CC_TYPES::SPHERE);
	scDlg.addType("    圆环", CC_TYPES::TORUS);
	scDlg.addType("    圆柱", CC_TYPES::CYLINDER);
	scDlg.addType("    圆锥", CC_TYPES::CONE);
	scDlg.addType("    立方", CC_TYPES::BOX);
	scDlg.addType("    圆盘", CC_TYPES::DISH);
	...
	scDlg.addType("图片", CC_TYPES::IMAGE);
	...
	scDlg.addType("八叉树", CC_TYPES::POINT_OCTREE);
