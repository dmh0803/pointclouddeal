...
// Default 'None' string
const char* ccPropertiesTreeDelegate::s_noneString = QT_TR_NOOP("无");

// Default color sources string
const char* ccPropertiesTreeDelegate::s_rgbColor = "RGB";
const char* ccPropertiesTreeDelegate::s_sfColor = QT_TR_NOOP("标量场");

// Other strings
const char* ccPropertiesTreeDelegate::s_defaultPointSizeString = QT_TR_NOOP("默认");
const char* ccPropertiesTreeDelegate::s_defaultPolyWidthSizeString = QT_TR_NOOP("默认宽");

// Default separator colors
constexpr const char* SEPARATOR_STYLESHEET = "QLabel { 背景色：深灰；颜色：白色； }";
...
void ccPropertiesTreeDelegate::fillModel(ccHObject* hObject)
{
	...
	if (m_model)
	{
		...
		m_model->setHeaderData(0, Qt::Horizontal, tr("属性"));
		m_model->setHeaderData(1, Qt::Horizontal, tr("状态/值"));
	}
...
void ccPropertiesTreeDelegate::fillWithHObject(ccHObject* _obj)
{
	...
	//name
	appendRow(ITEM(tr("名称")), ITEM(_obj->getName(), Qt::ItemIsEditable, OBJECT_NAME));

	//visibility
	if (!_obj->isVisiblityLocked())
		appendRow(ITEM(tr("显示")), CHECKABLE_ITEM(_obj->isVisible(), OBJECT_VISIBILITY));

	//normals
	if (_obj->hasNormals())
		appendRow(ITEM(tr("法线")), CHECKABLE_ITEM(_obj->normalsShown(), OBJECT_NORMALS_SHOWN));

	//name in 3D
	appendRow(ITEM(tr("显示名称(视图中)")), CHECKABLE_ITEM(_obj->nameShownIn3D(), OBJECT_NAME_IN_3D));

	//color source
	if (_obj->hasColors() || _obj->hasScalarFields())
		appendRow(ITEM(tr("颜色")), PERSISTENT_EDITOR(OBJECT_COLOR_SOURCE), true);
	...
	//Bounding-box
	{
		...
		if (box.isValid())
		{
			...
			appendRow(ITEM(fitBBox ? tr("局部范围") : tr("范围")),
			...
			appendRow(ITEM(shiftedObj ? tr("变换中心") : tr("中心")),
	...
	appendRow(ITEM(tr("其他信息")), ITEM(tr("对象ID: %1 - 子对象: %2").arg(_obj->getUniqueID()).arg(_obj->getChildrenNumber())));

	//display window
	if (!_obj->isLocked())
		appendRow(ITEM(tr("当前显示")), PERSISTENT_EDITOR(OBJECT_CURRENT_DISPLAY), true);
}
...
void ccPropertiesTreeDelegate::fillWithShifted(const ccShiftedObject* _obj)
{
	...
	appendRow(ITEM(tr("全局变换")), ITEM(QStringLiteral("(%1;%2;%3)").arg(shift.x, 0, 'f', 2).arg(shift.y, 0, 'f', 2).arg(shift.z, 0, 'f', 2)));

	double scale = _obj->getGlobalScale();
	appendRow(ITEM(tr("全局范围")), ITEM(QStringLiteral("%1").arg(scale, 0, 'f', 6)));
}
...
void ccPropertiesTreeDelegate::fillWithPointCloud(ccGenericPointCloud* _obj)
{
	...
	//number of points
	appendRow(ITEM(tr("点数")), ITEM(QLocale(QLocale::English).toString(_obj->size())));

	//global shift & scale
	fillWithShifted(_obj);

	//custom point size
	appendRow(ITEM(tr("点尺寸")), PERSISTENT_EDITOR(OBJECT_CLOUD_POINT_SIZE), true);
	...
	if (_obj->isA(CC_TYPES::POINT_CLOUD))
	{
		...
		{
			if (gridCount != 1)
				addSeparator(tr("扫描网格群"));
			else
				addSeparator(tr("扫描网格"));

			for (size_t i = 0; i < gridCount; ++i)
			{
				//grid size + valid point count
				ccPointCloud::Grid::Shared grid = cloud->grid(i);
				appendRow(ITEM(tr("扫描 #%1").arg(i + 1)), ITEM(tr("%1 x %2 (%3 points)").arg(grid->w).arg(grid->h).arg(QLocale(QLocale::English).toString(grid->validCount))));
			}
		}
...
void ccPropertiesTreeDelegate::fillWithPrimitive(const ccGenericPrimitive* _obj)
{
	...
	addSeparator(tr("原始图形"));

	//type
	appendRow(ITEM(tr("类型")), ITEM(_obj->getTypeName()));

	//drawing steps
	if (_obj->hasDrawingPrecision())
	{
		appendRow(ITEM(tr("绘制精度")), PERSISTENT_EDITOR(OBJECT_PRIMITIVE_PRECISION), true);
	}

	if (_obj->isA(CC_TYPES::SPHERE))
	{
		appendRow(ITEM(tr("半径")), PERSISTENT_EDITOR(OBJECT_SPHERE_RADIUS), true);
	}
	else if (_obj->isKindOf(CC_TYPES::CONE)) //cylinders are also cones!
	{
		appendRow(ITEM(tr("高度")), PERSISTENT_EDITOR(OBJECT_CONE_HEIGHT), true);
		if (_obj->isA(CC_TYPES::CYLINDER))
		{
			appendRow(ITEM(tr("半径")), PERSISTENT_EDITOR(OBJECT_CONE_BOTTOM_RADIUS), true);
		}
		else
		{
			appendRow(ITEM(tr("底部半径")), PERSISTENT_EDITOR(OBJECT_CONE_BOTTOM_RADIUS), true);
			appendRow(ITEM(tr("顶部半径")), PERSISTENT_EDITOR(OBJECT_CONE_TOP_RADIUS), true);
		}
	}
...
void ccPropertiesTreeDelegate::fillWithFacet(const ccFacet* _obj)
{
	...
	//surface
	appendRow(ITEM(tr("外观")), ITEM(QLocale(QLocale::English).toString(_obj->getSurface())));

	//RMS
	appendRow(ITEM(tr("RMS")), ITEM(QLocale(QLocale::English).toString(_obj->getRMS())));

	//center
	appendRow(ITEM(tr("中心")), ITEM(QStringLiteral("(%1 ; %2 ; %3)").arg(_obj->getCenter().x).arg(_obj->getCenter().y).arg(_obj->getCenter().z)));

	//contour visibility
	if (_obj->getContour())
		appendRow(ITEM(tr("显示等高线")), CHECKABLE_ITEM(_obj->getContour()->isVisible(), OBJECT_FACET_CONTOUR));

	//polygon visibility
	if (_obj->getPolygon())
		appendRow(ITEM(tr("显示多边形")), CHECKABLE_ITEM(_obj->getPolygon()->isVisible(), OBJECT_FACET_MESH));
}
...
void ccPropertiesTreeDelegate::fillWithPlanarEntity(const ccPlanarEntityInterface* _obj)
{
	...
	appendRow(ITEM(tr("法线")), ITEM(QStringLiteral("(%1 ; %2 ; %3)").arg(N.x).arg(N.y).arg(N.z)));
	...
	appendRow(ITEM(tr("显示法向量")), CHECKABLE_ITEM(_obj->normalVectorIsShown(), OBJECT_PLANE_NORMAL_VECTOR));
}
...
void ccPropertiesTreeDelegate::fillWithImage(const ccImage* _obj)
{
	...
	//image width
	appendRow(ITEM(tr("宽")), ITEM(QString::number(_obj->getW())));

	//image height
	appendRow(ITEM(tr("长")), ITEM(QString::number(_obj->getH())));

	//transparency
	appendRow(ITEM(tr("透明度")), PERSISTENT_EDITOR(OBJECT_IMAGE_ALPHA), true);
...
void ccPropertiesTreeDelegate::fillWithViewportObject(const cc2DViewportObject* _obj)
{
	...
	//Name
	appendRow(ITEM(tr("名称")), ITEM(_obj->getName().isEmpty() ? tr("undefined") : _obj->getName()));

	//"Apply Viewport" button
	appendRow(ITEM(tr("应用视图")), PERSISTENT_EDITOR(OBJECT_APPLY_LABEL_VIEWPORT), true);

	//"Update Viewport" button
	appendRow(ITEM(tr("更新视图")), PERSISTENT_EDITOR(OBJECT_UPDATE_LABEL_VIEWPORT), true);
	...
	const QModelIndex &index) const
{
	...
	{
		QPushButton* button = new QPushButton(tr("应用"), parent);
	...
	{
		QPushButton* button = new QPushButton(tr("更新"), parent);
