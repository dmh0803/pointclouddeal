		...
		for (size_t i = 0; i < selNum; ++i)
		{
			...
			if (entities[i]->isKindOf(CC_TYPES::POINT_CLOUD))
			{
				...
				if (cloud->isA(CC_TYPES::POINT_CLOUD))
				{
					...
					{
						ccConsole::Error(QString("未能在点云 '%1'上创建标量字段 (内存不足?)").arg(pc->getName()));
						continue;
					}
				}
				...
				if (!octree)
				{
					...
					if (!octree)
					{
						ccConsole::Error(QString("不能计算点云的'%1'八叉树!").arg(cloud->getName()));
						break;
					}
				}
