...
void qPoissonRecon::doAction()
{
	...
	{
		m_app->dispToConsole("只选中一个点云!", ccMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}
	...
	ccPointCloud* newPC = new ccPointCloud("顶点");
	...
	{
		//start message
		m_app->dispToConsole(QString("[泊松重构] 开始 (层次 %1)").arg(s_params.depth), ccMainAppInterface::STD_CONSOLE_MESSAGE);

		//progress dialog (Qtconcurrent::run can't be canceled!)
		QProgressDialog pDlg("初始化", QString(), 0, 0, m_app->getMainWindow());
		pDlg.setWindowTitle("泊松重构");
		pDlg.show();
		//QApplication::processEvents();

		QString progressLabel("正在进行重构\n");
		if (s_depthMode)
			progressLabel += QString("层次: %1").arg(s_params.depth);
		else
			progressLabel += QString("分辨率: %1").arg(s_params.finestCellWidth);
		progressLabel += QString(" [%1 线程]").arg(s_params.threads);
		...
		if (s_params.density)
		{
			s_densitySF = (densitySF = new ccScalarField("密度"));
		}
	...
	if (!result)
	{
		...
		delete newMesh;
		newMesh = nullptr;
		m_app->dispToConsole("重建失败!", ccMainAppInterface::ERR_CONSOLE_MESSAGE);
		return;
	}

	//success message
	m_app->dispToConsole(QString("[泊松重建] 完成(%1 三角, %2 顶点)").arg(newMesh->size()).arg(newPC->size()), ccMainAppInterface::STD_CONSOLE_MESSAGE);

	newMesh->setName(QString("模型[%1] (层次 %2)").arg(pc->getName()).arg(s_params.depth));
